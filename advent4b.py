...
inData = """sayndz zfxlkl attjtww cti sokkmty brx fhh suelqbp
...
for pwd in inData.split('\n'):
    ...
    for part in pwd.split(' '):
        ...
        for letter in part:
            myDict2[letter] += 1
        hashDict = tuple(zip(myDict2.keys(), myDict2.values()))
        myDict[hashDict] += 1
        if myDict[hashDict] > 1 or partSet in setList:
            validPwd = False
        setList.append(partSet)
    print(validPwd)
    if validPwd:
        validNo += 1
print(validNo)
